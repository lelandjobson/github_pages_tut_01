...
define(["jquery",], function($) {
   ...
   *
   * @param {String} path
   * @return {Object} exportsOvr
   * @api public
   */

  function requireScLoader(path, parent, orig) {
    var resolved = requireScLoader.resolve(path);

    // lookup failed
    if (null == resolved) {
      orig = orig || path;
      parent = parent || 'root';
      var err = new Error('Failed to requireScLoader "' + orig + '" from "' + parent + '"');
      err.path = orig;
      err.parent = parent;
      err.requireScLoader = true;
      throw err;
    }

    var module = requireScLoader.modules[resolved];

    // perform real requireScLoader()
    // by invoking the module's
    // registered function
    if (!module._resolving && !module.exportsOvr) {
      var mod = {};
      mod.exportsOvr = {};
      mod.client = mod.component = true;
      module._resolving = true;
      module.call(this, mod.exportsOvr, requireScLoader.relative(resolved), mod);
      delete module._resolving;
      module.exportsOvr = mod.exportsOvr;
    }

    return module.exportsOvr;
  }
  ...
  requireScLoader.modules = {};

  /**
   * Registered aliases.
   */

  requireScLoader.aliases = {};
  ...
  requireScLoader.resolve = function(path) {
    ...
    for (var i = 0; i < paths.length; i++) {
      var path = paths[i];
      if (requireScLoader.modules.hasOwnProperty(path)) return path;
      if (requireScLoader.aliases.hasOwnProperty(path)) return requireScLoader.aliases[path];
    }
  };
  ...
  requireScLoader.normalize = function(curr, path) {
    var segs = [];
  ...
  requireScLoader.register = function( path, definition) {
    requireScLoader.modules[path] = definition;
  };
  ...
  requireScLoader.alias = function(from, to) {
    if (!requireScLoader.modules.hasOwnProperty(from)) {
      throw new Error('Failed to alias "' + from + '", it does not exist');
    }
    requireScLoader.aliases[to] = from;
  };

  /**
   * Return a requireScLoader function relative to the `parent` path.
   *
   * @param {String} parent
   * @return {Function}
   * @api private
  ...
  requireScLoader.relative = function(parent) {
    var p = requireScLoader.normalize(parent, '..');
    ...
    /**
     * The relative requireScLoader() itself.
     */

    function localRequire(path) {
      //var resolved = localRequire.resolve(path);
      return requireScLoader(path, parent, path);
    }
    ...
    localRequire.resolve = function(path) {
      ...
      if ('.' == c) return requireScLoader.normalize(p, path);
    ...
    localRequire.exists = function(path) {
      return requireScLoader.modules.hasOwnProperty(localRequire.resolve(path));
    };

    return localRequire;
  };
  requireScLoader.register("side-comments/js/vendor/lodash-custom.js", function(exportsOvr, requireScLoader, module){
    /**
     * @license
     * Lo-Dash 2.4.1 (Custom Build) <http://lodash.com/>
     * Build: `lodash exportsOvr="node,commonjs" include="each,bind,find,template,reject,clone,cloneDeep"`
    ...
    ;(function() {
      ...
      /** Detect free variable `exportsOvr` */
      var freeExports = objectTypes[typeof exportsOvr] && exportsOvr && !exportsOvr.nodeType && exportsOvr;
  
      /** Detect free variable `module` */
      var freeModule = objectTypes[typeof module] && module && !module.nodeType && module;
  
      /** Detect the popular CommonJS extension `module.exportsOvr` */
      var moduleExports = freeModule && freeModule.exportsOvr === freeExports && freeExports;
       ...
       * don't requireScLoader escaping in HTML and have no special meaning unless they're part
      ...
      function template(text, data, options) {
        ...
        text.replace(reDelimiters, function(match, escapeValue, interpolateValue, esTemplateValue, evaluateValue, offset) {
          ...
          // the JS engine embedded in Adobe products requireScLoaders returning the `match`
      ...
      if (freeExports && freeModule) {
        ...
        if (moduleExports) {
          (freeModule.exportsOvr = lodash)._ = lodash;
        }
        // in Narwhal or Rhino -requireScLoader
        else {
  ...
  requireScLoader.register("component-emitter/index.js", function(exportsOvr, requireScLoader, module){
  ...
  module.exportsOvr = Emitter;

  });
  requireScLoader.register("side-comments/js/helpers/mobile-check.js", function(exportsOvr, requireScLoader, module){
    module.exportsOvr = function() {
      var check = false;
  ...
  requireScLoader.register("side-comments/js/section.js", function(exportsOvr, requireScLoader, module){
    ...
    */
    ...
    module.exportsOvr = Section;
    });
  requireScLoader.register("side-comments/js/main.js", function(exportsOvr, requireScLoader, module){
  var _ = requireScLoader('side-comments/js/vendor/lodash-custom.js');
  var Section = requireScLoader('side-comments/js/section.js');
  var Emitter = requireScLoader('component-emitter/index.js');
  var $ = jQuery;
  ...
  module.exportsOvr = SideComments;

  });
  requireScLoader.alias("component-emitter/index.js", "side-comments/deps/emitter/index.js");
  requireScLoader.alias("component-emitter/index.js", "emitter/index.js");

  requireScLoader.alias("side-comments/js/main.js", "side-comments/index.js");
  
  // initialize
  requireScLoader.modules["side-comments/js/main.js"]({},requireScLoader,mods);

  return mods.exportsOvr;
});